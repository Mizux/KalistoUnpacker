...
struct [[gnu::packed]] kixNode_t {
 public:
  ...
  friend std::ostream& operator<<(std::ostream& os, const kixNode_t& node) {
    os << "{Type: '" << node.type << "' " << std::hex << ", memAddr: 0x"
       << node.memAddr << ", offset: 0x" << node.offset << std::dec << " ("
       << node.size << "bytes)}";
    return os;
  }
};
void readKixNode(std::ifstream& kix, kixNode_t* node,
...
void readKbfNode(std::ifstream& kbf, kbfNode_t* node,
                 bool advance_cursor = false);
}  // namespace ku
